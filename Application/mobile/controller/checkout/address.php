...
class ControllerCheckoutAddress extends Controller {
    public function index() {
        $lang =$this->language->load('checkout/address');
        ...
        if ($this->customer->isLogged()) {
            ...
            if (isset($this->session->data['shipping_address'])) {
                ...
                $this->redirect($this->url->link('account/address/update', '', 'SSL'));
            } else {
                $is_empty_address = 1;
                $this->data['shipping_address_list'] = null;
                $this->session->data['shipping_address'] = null;
        ...
        $this->data['checked_address'] = $this->session->data['shipping_address'];
         if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/include/shipping_address_list.tpl')) {
    ...
    public function saveBillingAddress() {
     ...
     public function changeAddress(){
        $json = array();
        $address_id = $this->request->post['address_id'];
        if($address_id  && $this->customer->isLogged()){
             $this->load->model('account/address');
             $shipping_address = $this->model_account_address->getAddress($address_id);
             $this->session->data['shipping_address'] = $shipping_address;
             $json['error'] = 0;
             ...
             $json['error'] = 1;
             $json['redirect'] = $this->url->link('checkout/address','','SSL');
        }
         $this->response->setOutput(json_encode($json));
    }
