<?php
class ModelAccountPoints extends Model {	
	//得到用户的积分记录
	public function getRewards($data = array(),$status) {
		...
		switch($status){
			...
			break;
			case 'available':
			$where .=' AND c.status =1 and  (c.points>c.points_spent ) ';
			break;
			case 'pending':
			$where .=' AND c.status =0 ';
			break;
			case 'used':
			$where .=' AND c.status =1 and  c.points_spent >0 ';
			break;
		}
		$sql.=$where;
		$sort_data = array(
