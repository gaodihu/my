...
class ModelShippingMyled extends Model {
    ...
    function getQuote($address,$products) {
        ...
        $airmail = $this->getMethodByCode('airmail', $country_code, $total_weigth);
        if ($airmail) {
            $method_data[] = $airmail;
        }

        $standard = $this->getMethodByCode('standard', $country_code, $total_weigth);
        if ($standard) {
        ...
        $expedited = $this->getMethodByCode('expedited', $country_code, $product_cal_weigth);
        if ($expedited) {
            //DHL的燃油附加费
            $_surcharge_1 = 0;
            $_surcharge_2 = 0;
        ...
        $ems = $this->getMethodByCode('ems', $country_code, $product_cal_weigth);
        if ($ems) {
            $method_data[] = $ems;
        }
        return $method_data;
