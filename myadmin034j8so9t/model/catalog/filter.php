...
class ModelCatalogFilter extends Model {
	public function addFilter($data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				$filter_attr_array = array();
				if($filter['filter_attr']){
					foreach ($filter['filter_attr'] as $filter_attr) {
						$filter_attr_array[] =$filter_attr;
					}
				}
				$filter_attrribute = implode(',',$filter_attr_array);
				$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "',attribute='".$filter_attrribute."', sort_order = '" . (int)$filter['sort_order'] . "'");
				...
				//插入分类filter
				foreach ($filter['filter_category'] as $filter_category) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "category_filter SET category_id = '" . (int)$filter_category . "', filter_id = '" . (int)$filter_id . "'");
				}
			}
		}		
	}
	...
	public function editFilter($filter_group_id, $data) {
		...
		if (isset($data['filter'])) {
			foreach ($data['filter'] as $filter) {
				if ($filter['filter_id']) {
					$filter_attrribute =$filter['filter_attr']? implode(',',$filter['filter_attr']):'';
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_id = '" . (int)$filter['filter_id'] . "', filter_group_id = '" . (int)$filter_group_id . "',attribute='".$filter_attrribute."', sort_order = '" . (int)$filter['sort_order'] . "'");
				} else {
					$this->db->query("INSERT INTO " . DB_PREFIX . "filter SET filter_group_id = '" . (int)$filter_group_id . "', attribute='".$filter_attrribute."', sort_order = '" . (int)$filter['sort_order'] . "'");
				}
	...
	public function getFilterDescriptions($filter_group_id) {
		...
		foreach ($filter_query->rows as $filter) {
			...
			$filter_category_query =$this->db->query("SELECT category_id FROM " . DB_PREFIX ."category_filter where filter_id = ". (int)$filter['filter_id'] );
			$filter_data[] = array(
				'filter_id'          => $filter['filter_id'],
				'filter_description' => $filter_description_data,
				'filter_attribute' => explode(',',$filter['attribute']),
				'filter_category' => $filter_category_query->rows,
	...
	public function getFilterCategory($filter_id){
		$query = $this->db->query("SELECT category_id FROM" . DB_PREFIX ."category_filter where filter_id = ".$filter_id);
		return $query->rows;
	}
}
?>
