...
class ModelShippingBattery extends Model {
    ...
    function getQuote($address,$products) {
        ...
        $standard = $this->getMethodByCode('standardwb', $country_code, $total_weigth);
        if ($standard) {
    ...
    public function getMethodTypeByCode($method_code){
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "shipping_matrixrate_battery WHERE delivery_method='". $method_code ."'  limit 1");
        if ($query->num_rows) {
			$status = true;
