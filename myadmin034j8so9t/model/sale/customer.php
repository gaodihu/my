...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 2)) . "', password = '" . $this->db->escape(md5($salt.$data['password']).":".$salt) . "', status = '" . (int)$data['status'] . "', date_added = NOW()");

		$customer_id = $this->db->getLastId();

		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "',type='".$this->db->escape($address['type'])."', company = '" . $this->db->escape($address['company']) . "', company_id = '" . $this->db->escape($address['company_id']) . "', tax_id = '" . $this->db->escape($address['tax_id']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "'");
	...
	public function editCustomer($customer_id, $data) {
		...
		if ($data['password']) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 2)) . "', password = '" . $this->db->escape(md5($salt.$data['password']).":".$salt) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
		...
		if (isset($data['address'])) {
			...
			foreach ($data['address'] as $address) {
                ...
                $address['country_code'] = $country_info['iso_code_2'];
				...
				$this->db->query("INSERT INTO  " . DB_PREFIX . "address SET address_id = '".(int)$address['address_id']."' ,customer_id='".(int)$customer_id."',firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "',type='".$this->db->escape($address['type'])."', company = '" . $this->db->escape($address['company']) . "', company_id = '" . $this->db->escape($address['company_id']) . "', tax_id = '" . $this->db->escape($address['tax_id']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '" . (int)$address['zone_id'] . "',country_code='".$this->db->escape($address['country_code'])."',zone_code='".$this->db->escape($address['zone_code'])."',zone_name='".$this->db->escape($address['zone'])."'" );
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			if ($zone_query->num_rows) {
                ...
                if ($address_query->row['zone_name']) {
	...
	public function getAddresses($customer_id,$type='') {
		$address_data = array();
        if($type=='billing'){
            $query = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "' and type='billing' ");
        }elseif($type=='shipping'){

            $query = $this->db->query("SELECT address_id FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$customer_id . "' and (type='shipping' or type is null or type='') ");
	...
	public function getRewards($customer_id, $start = 0, $limit = 10) {
		$query = $this->db->query("SELECT cr.*,o.order_number,o.order_status_id FROM " . DB_PREFIX . "customer_reward as cr left join ".DB_PREFIX."order as o on cr.order_id=o.order_id WHERE cr.customer_id = '" . (int)$customer_id . "' ORDER BY cr.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
    ...
    public function getCustomerGroupInfoByPoints($points){
        $query = $this->db->query("SELECT customer_group_id FROM " . DB_PREFIX . "customer_group WHERE point <= '" . $points ."' order by point desc limit 1");
        return $query->row['customer_group_id'];
    }

	public function getRewardTotal($customer_id) {
		$query = $this->db->query("SELECT SUM(points) AS total FROM " . DB_PREFIX . "customer_reward WHERE customer_id = '" . (int)$customer_id . "' and status=1");
