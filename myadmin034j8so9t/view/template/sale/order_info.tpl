...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      ...
      <div class="buttons">
	   <?php if($send_email){ ?>
	 <!-- <a href="<?php echo $send_email; ?>"  class="button">Send Email</a>-->
	  <?php } ?>
	  <?php if(isset($send_pending_email)){ ?>
	  <a href="<?php echo $send_pending_email; ?>"  class="button">Send Pending Email</a>
	   <?php if(isset($send_pending_email_date)){ ?>
	  (<?php echo $send_pending_email_date;?>)
	  <?php } ?>
	  <?php } ?>
	  <?php if(isset($send_credit_validate_email)){ ?>
	  <a href="<?php echo $send_credit_validate_email; ?>"  class="button">Send Credit Validate Email</a>
	  <?php if(isset($send_credit_validate_email_date)){ ?>
	  (<?php echo $send_credit_validate_email_date;?>)
	  <?php } ?>
	  <?php } ?>
	  <?php if($shippment){ ?>
	  <a href="<?php echo $shippment; ?>"  class="button">Creat Shippment</a>
	  <?php } ?>
	  <?php if($invoice_print){ ?>
	  <a href="<?php echo $invoice_print; ?>" target="_blank" class="button">Print Invoice</a>
	  <?php } ?>
	  <?php if($invoice_add){ ?>
	  <a href="<?php echo $invoice_add; ?>"  class="button">Creat Invoice</a>
	  <?php } ?>
    ...
    <div class="content">
      <div class="vtabs">
	  	<a href="#tab-order"><?php echo $tab_order; ?></a>
             ...
             <a href="#sub-orders">Sub Orders</a>
        <?php } ?>
        
        <?php if($parent_order && count($parent_order)>0) {?>
             <a href="#main-orders">Main Order</a>
        <?php } ?>
        ...
        <?php } ?>
		<?php if ($shippment_list) { ?>
        <a href="#tab-shippment">Shippment List</a>
        <?php } ?>
        ...
        <table class="form">
          ...
          <tr>
            ...
            <td><?php if ($invoice_no) { ?>
			  <a href="<?php echo $invoice_url; ?>"><?php echo $invoice_no; ?></a>
              <?php } else { ?>
             <!-- <span id="invoice"><b>[</b> <a id="invoice-generate"><?php echo $text_generate; ?></a> <b>]</b></span>-->
			 <span><b>[</b> <?php echo $text_generate; ?> <b>]</b></span>
              <?php } ?></td>
          </tr>
          <tr>
          ...
          <tr>
            <td>Guest</td>
            <td> <?php if ($customer) { ?>No<?php } else {  ?>Yes<?php } ?></td>
          </tr>
          
          <?php if ($customer) { ?>
          <tr>
            <td><?php echo $text_customer; ?></td>
            <td><a href="<?php echo $customer; ?>"><?php echo $email; ?></a></td>
          </tr>
          <?php } else { ?>
          <tr>
            <td><?php echo $text_customer; ?></td>
          ...
          <tr>
            ...
            <td><?php echo $total; ?>(<?php echo $base_total;?>)
      ...
      <div id="tab-payment" class="vtabs-content">
        <table class="form">
          ...
          <?php if($order_info['payment_code'] == 'pp_onestep'){ ?>
          <tr>
              <td>paypal 1-step 授权后购买</td>
              <td><?php if($paypal_onestep_auth) { ?>是<?php }else { ?>否<?php }?></td>
          </tr>
          <?php } ?>
        </table>
          <?php if($paypal_data){ ?>
           <table>
          <?php foreach($paypal_data as $key => $val){ ?>
          <tr>
            <td><?php echo $key ?></td>
            <td><?php echo $val; ?></td>
          </tr>
      ...
      <?php } ?>
	  <?php if ($shippment_list) { ?>
      <div id="tab-shippment" class="vtabs-content">
       <table class="list">
          <thead>
            <tr>
              <td class="left">shippment No#</td>
	          <td class="left">shippment to name</td>
              <td class="left">shippment track info</td>
			  <td class="left">shippment date</td>
			  <td class="left">action</td>
            </tr>
          </thead>
          <tbody>
            <?php foreach ($shippment_list as $shippment) { ?>
            <tr>
              <td class="left"><a href="<?php echo $shippment['href']; ?>"><?php echo $shippment['shippment_number']; ?></a></td>
	          <td class="left"><?php echo $shippment['customer']; ?></td>
			  <td class="left">
			  <?php foreach($shippment['track_info'] as $track_info){ ?>
			  track title:<?php echo $track_info['title'];?>;track_number:<?php echo $track_info['track_number'];?><br>
			  <?php } ?>
			  </td>
              <td class="left"><?php echo $shippment['created_at']; ?></td>
			  <td class="left"><a href="<?php echo $shippment['delete'];?>" class='button'>delete</a></td>
            </tr>
			<tr>
            <?php } ?>
          </tbody>
        ...
        <table class="list">
          <thead>
            <tr>
              ...
              <td class="right"><?php echo 'original price'; ?></td>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="right"><?php echo $product['original_price']; ?></td>
              <td class="right"><?php echo $product['price']; ?>(<?php echo $product['base_price'];?> )</td>
              <td class="right"><?php echo $product['total']; ?>(<?php echo $product['base_total'];?> )</td>
            </tr>
            <?php } ?>
          ...
          <tbody id="totals">
            <tr>
              ...
              <td class="right"><?php echo $order_info['currency_code']; ?><?php echo $totals['text']; ?><br />(USD$<?php echo $totals['value'];?>)</td>
            </tr>
          </tbody>
          <?php } ?>
        </table>
      ...
      <div id="sub-orders" class="vtabs-content">
        <?php if($children && is_array($children)){ ?>
			  ...
			  <td class="left"><?php echo $_c['order_number']; ?></td>
                        ...
                        <td class="right" style="word-break: keep-all;white-space:nowrap;">
                        <?php foreach ($_c['action'] as $action) { ?>
      ...
      <?php if($parent_order && count($parent_order)) { ?>
        <div id="main-orders" class="vtabs-content">
        <table width="100%"  cellpadding="0" cellspacing="0" border="0" class="form">
            <tr><td colspan='9'><b>Main Order:</b></td></tr>
             <tr>
                <td class="right"><?php echo $parent_order['order_id']; ?></td>
                <td class="left"><?php echo $parent_order['order_number']; ?></td>
                <td class="left"><?php echo $parent_order['status']; ?></td>
                <td class="left"><?php echo $parent_order['payment_code']; ?></td>
                <td class="right"><?php echo $parent_order['base_total']; ?></td>
                <td class="right"><?php echo $parent_order['currency_code']; ?><?php echo $parent_order['total']; ?></td>
                <td class="left"><?php echo $parent_order['date_added']; ?></td>
                <td class="right" style="word-break: keep-all;white-space:nowrap;">
                <?php foreach ($parent_order['action'] as $action) { ?>
                  [ <a href="<?php echo $action['href']; ?>"><?php echo $action['text']; ?></a> ]
                  <?php } ?></td>
              </tr>
               
      
                <?php if($parent_order['children'] && is_array($parent_order['children'])){ ?>
                <tr><td colspan='9'><b>Sub Orders:</b></td></tr>
                 <tr >
                  <td colspan='9'>
                  <table width="100%"  cellpadding="0" cellspacing="0" border="0" class="form">
                    <?php foreach($parent_order['children'] as $_c) { ?>
