...
class ControllerSaleOrder extends Controller {
    ...
    private $payment_code = array(
        'paypal' => array('pp_standard','pp_express','pp_onestep'),
        'westernunion' => array('westernunion'),
        'bank_transfer' => array('bank_transfer'),
        'sofortbanking' => array('sofortbanking_payment','globebill_sofortbanking'),
        'giropay' => array('giropay_payment','globebill_giropay'),
        'safety' => array('safety_payment','globebill_safety'),
        'boleto' => array('boleto_payment','globebill_boleto','tef_payment'),
        'qiwi' => array('qiwi_payment','globebill_qiwi'),
        'webmoney' => array('webmoney_payment','globebill_webmoney'),
        'poli' => array('poli_payment','globebill_poli'),
        'credit' => array('globebill_credit'),
    );
	public function index() {
        ...
        if(isset($this->session->data[$user_id."_new_order"])){
            unset($this->session->data[$user_id."_new_order"]);
        }
		$this->getList();
	}
	...
	public function insert() {
       ...
       if (($this->request->server['REQUEST_METHOD'] != 'POST')){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $address_info =array();
            $address_info['firstname'] =isset($this->request->post['shipping_firstname'])?$this->request->post['shipping_firstname']:'';
            $address_info['lastname'] =isset($this->request->post['shipping_lastname'])?$this->request->post['shipping_lastname']:'';
            $address_info['company'] =isset($this->request->post['shipping_company'])?$this->request->post['shipping_company']:'';
            $address_info['tax_id'] =isset($this->request->post['order_tax_id'])?$this->request->post['order_tax_id']:'';
            $address_info['address_1'] =isset($this->request->post['shipping_address_1'])?$this->request->post['shipping_address_1']:'';
            $address_info['address_2'] =isset($this->request->post['shipping_address_2'])?$this->request->post['shipping_address_2']:'';
            $address_info['postcode'] =isset($this->request->post['shipping_postcode'])?$this->request->post['shipping_postcode']:'';
            $address_info['city'] =isset($this->request->post['shipping_city'])?$this->request->post['shipping_city']:'';
            $address_info['zone_id'] =isset($this->request->post['shipping_zone_id'])?(int)$this->request->post['shipping_zone_id']:'';
            $address_info['phone'] =isset($this->request->post['shipping_phone'])?(int)$this->request->post['shipping_phone']:'';
            $address_info['country_id'] =isset($this->request->post['shipping_country_id'])?(int)$this->request->post['shipping_country_id']:'';
       
            $this->load->model('localisation/country');
            $country_info = $this->model_localisation_country->getCountry($address_info['country_id']);
            $address_info['iso_code_2'] =$country_info['iso_code_2'];
            $this->load->model('shipping/myled');
            
            $total_weigth = 0;
            $products =$this->session->data[$user_id."_new_order"]['order_products'];
            foreach($products as $pro){
                $total_weigth +=$pro['weight'] * $pro['quantity'];
            }
            $_all_packages = $this->model_shipping_myled->getShippingMethod($address_info,$total_weigth,$products);
            ...
            if(count($_all_packages['no_shipping'])>0){
                ...
                $redirect = $this->url->link('sale/order/insert', '', 'SSL');
                $this->redirect($redirect);
            }
            
            //$total_data =$this->request->post['order_total'];
            ///////////////////////////////////生产主订单/////////////////////////////////////
            /*
            ...
            if(count($shipping_packages)>1){
                ...
                //bug fix 处理的问题
                $_delivery_method = $this->request->post['shipping']['p1'];
              
                if($shipping_packages['p1']['battery']==1){
                    $this->load->model('shipping/battery');
                    $order_data['shipping_method'] =$this->model_shipping_battery->getMethodTypeByCode($_delivery_method);
                }else{
                    $order_data['shipping_method'] =$this->model_shipping_myled->getMethodTypeByCode($_delivery_method);
                }
                
                $order_data['shipping_code']   = $_delivery_method;
            ...
            $insert_id = $this->model_sale_order->addOrder($order_data);
            ///////////////////////////分包代码////////////////////////////////////////////
            //分包代码，生产
            if(count($shipping_packages)>1){
                foreach($shipping_packages as $_pk => $_package ){
                    $_package_data =$this->request->post;
                    $select_shipping = $this->request->post['shipping'];
                    if (isset($this->request->post['shipping'][$_pk])) {
                        if($_package['battery']==1){
                            $this->load->model('shipping/battery');
                            $_package_data['shipping_method'] =$this->model_shipping_battery->getMethodTypeByCode($this->request->post['shipping'][$_pk]);
                        }else{
                            $_package_data['shipping_method'] =$this->model_shipping_myled->getMethodTypeByCode($this->request->post['shipping'][$_pk]);
                        }
                        $_package_data['shipping_code'] = $this->request->post['shipping'][$_pk];
                        $_package_data['shipping_cost'] = $_package['methods'][$this->request->post['shipping'][$_pk]]['price'];
                    } else {
                        $_package_data['shipping_method'] = '';
                        $_package_data['shipping_code'] = '';
                        $_package_data['shipping_cost'] = 0;
                    }
                    ...
                    foreach ($_package['package'] as $product) {
                        ...
                        $_package_product_data[] = array(
                            ...
                            'price' => $product['price'],
                            'price_format' => $this->currency->format($product['price'], $product['tax_class_id'], $this->config->get('config_tax')),
                            'trie_price' => $product['trie_price'],
                            'special_price' => $product['special_price'],
                            'total' => $product['base_total'],
                    ...
                    $_package_data['order_product'] = $_package_product_data;
                    
                    
                    
                    $_package_data['comment'] = '';
                    $order_totals =array();
                    $subtotal=$_package_total;
                    $sub_text =$this->currency->format($subtotal,$this->session->data[$user_id."_new_order"]['curreny_code']);       
                    $order_totals[] =array(
                        'code' =>'sub_total', 
                        'title' =>'Subtotal', 
                        'text' =>$sub_text,
                    ...
                    $shipping_total =$_package_data['shipping_cost'];
                    $shipping_text =$this->currency->format($shipping_total,$this->session->data[$user_id."_new_order"]['curreny_code']);    
                    $order_totals[] =array(
                        'code' =>'shipping', 
                        'title' =>$_package_data['shipping_method'], 
                        'text' =>$shipping_text,
                        'value' =>$shipping_total,
                        'sort_order' =>'3',
                    );
                    $grand_tota =$subtotal+$shipping_total;
                    $grand_tota_text =$this->currency->format($grand_tota,$this->session->data[$user_id."_new_order"]['curreny_code']); 
                    $order_totals[] =array(
                        'code' =>'total', 
                        'title' =>'Grand Total', 
                        'text' =>$grand_tota_text,
                        'value' =>$grand_tota,
                        'sort_order' =>'9',
                    );
                    $_package_data['order_total'] =$order_totals;
                    $_package_data['is_parent'] = '0';
                    $_package_data['parent_id'] = $insert_id;
                    $_pack_order_id = $this->model_sale_order->addOrder($_package_data);
                }
    ...
    public function update_send() {
		...
		if (isset($this->request->post['selected'])) {
			foreach ($this->request->post['selected'] as $order_id) {
                ...
                if($order_info){
                    $this->model_sale_order->UpdateSendStatus($order_id);
                }
			}
	...
	protected function getList() {
		...
		$this->data['update_send'] = $this->url->link('sale/order/update_send', 'token=' . $this->session->data['token'], 'SSL');
        $this->data['shippment'] = $this->url->link('sale/shippment/upload', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->data['join'] = $this->url->link('sale/order_action/join', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		*/
		$data = $filter;
        $data['limit'] = isset($this->request->get['limit'])?$this->request->get['limit']:$this->config->get('config_admin_limit');
		$data['start'] = ($filter['page'] - 1) * $data['limit'];
        $this->data['current_filter_payment_code'] = '';
        if(isset($data['filter_payment_code']) && $data['filter_payment_code']){
            $payment_code_arr = $this->payment_code[$data['filter_payment_code']];
            $payment_code_str = '';
            foreach($payment_code_arr as $item){
                $payment_code_str .= "'" . $item ."',";
            }
            if($payment_code_str){
                $payment_code_str = substr($payment_code_str,0,-1);
                $this->data['current_filter_payment_code'] = $data['filter_payment_code'];
                $data['filter_payment_code'] = $payment_code_str;
            }
        }else{
            unset($data['filter_payment_code']);
        }
        $store_array =array();
        $this->load->model('setting/store');
        $store_array[0]='www.myled.com';    
		$stores= $this->model_setting_store->getStores();
        foreach($stores as $store){
             $store_array[$store['store_id']] =$store['url'];
        }
        ...
        $send_status =array('0'=>'Not Send','1'=>'Send','2'=>'Send&Purchased');
		foreach ($results as $result) {
			...
			if ($result['is_parent'] == 0 && strtotime($result['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				$action[] = array(
            ...
            if($result['is_parent'] == 1){
                $_children = $this->model_sale_order->getOrders(array('parent_id'=>$result['order_id']));
                foreach($_children as $_item ){
                    ...
                    $_action[] = array(
                        'text' => $this->language->get('text_view'),
                        'href' => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $_item['order_id'] , 'SSL')
                    );
                    
                    if (strtotime($_item['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
                        $_action[] = array(
                            'text' => $this->language->get('text_edit'),
                            'href' => $this->url->link('sale/order/update', 'token=' . $this->session->data['token'] . '&order_id=' . $_item['order_id'] . $url, 'SSL')
                        );
                    }
                    ...
                    $_children_data[] = array(
                        ...
                        'store_from'      =>$store_array[$_item['store_id']],
                        'send_status'      => $send_status[$_item['send_forecast_status']],
                        'order_number'      => $_item['order_number'],
                        'customer'      => $_item['customer'],
                        'email'        => $_item['email'],
                        'status'        => $_item['status'],
                        'currency_code'        => $_item['currency_code'],
                        'base_total'         => $this->currency->format($_item['total'],$this->config->get('config_currency')),
                        'total'         =>  $this->currency->onlyFormat($_item['grand_total'],$_item['currency_code']),
                        'date_added'    => date($this->language->get('date_format_short'), strtotime($_item['date_added'])),
                        'date_modified' => date($this->language->get('date_format_short'), strtotime($_item['date_modified'])),
                        'selected'      => isset($this->request->post['selected']) && in_array($_item['order_id'], $this->request->post['selected']),
                        'action'        => $_action,
                        'payment_method' => $_item['payment_method'],
                        'payment_code' => $_item['payment_code'],
                    );
                }
            }
			...
			$this->data['orders'][] = array(
                ...
                'store_from'      =>$store_array[$result['store_id']],
                'send_status'      => $send_status[$result['send_forecast_status']],
				'order_number'      => $result['order_number'],
				'customer'      => $result['customer'],
                'email'        => $result['email'],
				'status'        => $result['status'],
                'currency_code'        => $result['currency_code'],
                'base_total'         => $this->currency->format($result['total'],$this->config->get('config_currency')),
				'total'         =>  $this->currency->onlyFormat($result['grand_total'],$result['currency_code']),
		...
		$sort_arr = array(
			'sort_order' => 'o.order_id',	
			'sort_customer' => 'customer',
			'sort_status' => 'status',
			'sort_total' => 'o.total',
			'sort_date_added' => 'o.date_added',
			'sort_date_modified' => 'o.date_modified'
		);
		foreach($sort_arr as $key =>$sort_value){
			$this->data[$key] = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . "&sort=$sort_value" . $url, 'SSL');
        ...
        $this->data['filter_payment_code'] = $this->payment_code;
        
		$this->load->model('localisation/order_status');

		$this->data['filter']['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();
	...
	public function getInsertForm() {
        ...
        if (isset($this->error['shipping_phone'])) {
			$this->data['error_shipping_phone'] = $this->error['shipping_phone'];
		} else {
			$this->data['error_shipping_phone'] = '';
		}
		...
		$this->url->getParUrl($this->base_filer);
		...
		$this->data['cancel'] = $this->url->link('sale/order/cancel', 'token=' . $this->session->data['token'] . $url, 'SSL');
        ...
        if (isset($this->request->post['order_tax_id'])) {
			$this->data['order_tax_id'] = $this->request->post['order_tax_id'];
		} elseif (!empty($order_info)) {
			$this->data['order_tax_id'] = $order_info['order_tax_id'];
		} else {
			$this->data['order_tax_id'] = '';
		}
        ...
        if (isset($this->request->post['shipping_phone'])) {
			$this->data['shipping_phone'] = $this->request->post['shipping_phone'];
		} elseif (!empty($order_info)) {
		...
		foreach ($order_products as $order_product) {
			...
			$this->data['order_products'][] = array(
                ...
                'currency_price'            => $order_product['currency_price'],
				'total'            => $order_product['total'],
                'currency_total'            => $order_product['currency_total'],
		...
		$this->template = 'sale/order_insert_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function getForm() {
         if(isset($this->session->data['new_order'])){
            unset($this->session->data['new_order']);
        }
		$this->load->model('sale/customer');
        ...
        $this->data['curreny_code'] =$order_info['currency_code'];
		$this->data['token'] = $this->session->data['token'];
		...
		foreach ($order_products as $order_product) {
			...
			$this->data['order_products'][] = array(
				...
				'quantity'         => $order_product['quantity'],
				'price'            => $order_product['price'],
                'curreny_price'            => $order_product['currency_price'],
				'total'            => $order_product['total'],
                'curreny_total'            => $order_product['currency_total'],
				'tax'              => $order_product['tax'],
				'reward'           => $order_product['reward'],
                'original_price'   => $order_product['original_price'],
			);
		}
	...
	protected function validateForm() {
        ...
        */
		...
		if ($shipping) {
			...
			if ((utf8_strlen($this->request->post['shipping_firstname']) < 0) || (utf8_strlen($this->request->post['shipping_firstname']) > 32)) {
				$this->error['shipping_firstname'] = $this->language->get('error_firstname');
			}

			if ((utf8_strlen($this->request->post['shipping_lastname']) < 0) || (utf8_strlen($this->request->post['shipping_lastname']) > 32)) {
				$this->error['shipping_lastname'] = $this->language->get('error_lastname');
			}

			if ((utf8_strlen($this->request->post['shipping_address_1']) < 0) || (utf8_strlen($this->request->post['shipping_address_1']) > 128)) {
				$this->error['shipping_address_1'] = $this->language->get('error_address_1');
			}

			if ((utf8_strlen($this->request->post['shipping_city']) < 0) || (utf8_strlen($this->request->post['shipping_city']) > 128)) {
			...
			if ($country_info && (utf8_strlen($this->request->post['shipping_postcode']) < 2) || (utf8_strlen($this->request->post['shipping_postcode']) > 10)) {
			...
			if (!$this->request->post['shipping']&&!$this->request->post['shipping_method']) {
	...
	public function info() {
		...
		if ($order_info) {
			$this->data['order_number'] = $order_info['order_number'];
			$this->language->load('sale/order');
            ...
            if(!$order_info['email_send']){
                $this->data['send_email'] = $this->url->link('sale/order/send_email', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
            }
            else{
                $this->data['send_email'] ='';
            }
            $this->data['invoice_print'] = $this->url->link('sale/invoice/info', 'print=1&token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
            if($order_info['invoice_id']){
                
                $this->data['invoice_add'] = '';
            }
            else{
              
                $this->data['invoice_add'] = $this->url->link('sale/invoice/add', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
            }
            if($order_info['order_status_id']==1){
                $this->data['send_pending_email'] = $this->url->link('sale/order/send_pending_email', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
                if($order_info['pending_email_send_time']){
                    $this->data['send_pending_email_date'] =$order_info['pending_email_send_time'];
                }
                
            }
            if($order_info['base_grand_total']>500&&$order_info['payment_code']=='globebill_credit'&&$order_info['order_status_id']==2){
                $this->data['send_credit_validate_email'] = $this->url->link('sale/order/credit_validate_email', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
                if($order_info['credit_validate_email_send_time']){
                    $this->data['send_credit_validate_email_date'] =$order_info['credit_validate_email_send_time'];
                }
                
            }

            
            $shippment_list =$this->model_sale_order->getOrderShippment($this->request->get['order_id']);
           
            $this->data['shippment'] = $this->url->link('sale/shippment/add', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
             $this->data['shippment_list'] =  array();
             $this->load->model('sale/shippment');
             foreach($shippment_list as $shippment){
                 $this->data['shippment_list'][] =array(
                    'shippment_id' =>  $shippment['shippment_id'],
                    'shippment_number'  =>$shippment['shippment_number'],
                    'track_info'                          =>$this->model_sale_shippment->getShippmentTrack($shippment['shippment_id']),
                    'created_at'  =>$shippment['created_at'],
                    'customer'  =>$order_info['shipping_firstname']." ".$order_info['shipping_lastname'],
                    'href' =>$this->url->link('sale/shippment/info', 'token=' . $this->session->data['token'] . '&shippment_id=' . (int)$shippment['shippment_id']. '&order_id=' . (int)$this->request->get['order_id'], 'SSL'),
                    'delete' =>$this->url->link('sale/shippment/delete', 'token=' . $this->session->data['token'] . '&shippment_id=' . (int)$shippment['shippment_id']. '&order_id=' . (int)$this->request->get['order_id'], 'SSL'),
                    'update' =>$this->url->link('sale/shippment/update', 'token=' . $this->session->data['token'] . '&shippment_id=' . (int)$shippment['shippment_id']. '&order_id=' . (int)$this->request->get['order_id'], 'SSL')
                 );
             }
            ...
            if ($order_info['invoice_id']) {
                $invoice_url =$this->url->link('sale/invoice/info','token=' . $this->session->data['token'].'&order_id='.$this->request->get['order_id']);
                $this->data['invoice_url'] =$invoice_url ;
			...
			$this->data['firstname'] = $order_info['firstname'];
			$this->data['lastname'] = $order_info['lastname'];
            $this->data['is_remote'] = $order_info['is_remote'];
			...
			$this->data['shipping_method'] = $order_info['shipping_method'];
			$this->data['payment_method'] = $order_info['payment_method'];
            $this->data['base_total'] = $this->config->get('config_currency') . $this->currency->format($order_info['total'], $this->config->get('config_currency'));
			$this->data['total'] = $order_info['currency_code'] . $this->currency->onlyFormat($order_info['grand_total'], $order_info['currency_code']);
            ...
            $this->data['shipping_phone'] = $order_info['shipping_phone'];
            $this->data['paypal_onestep_auth'] = $order_info['paypal_onestep_auth'];
			...
			foreach ($products as $product) {
				...
				$this->data['products'][] = array(
					...
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
                    'original_price'   => $this->config->get('config_currency') . $this->currency->onlyFormat($product['original_price'],$this->config->get('config_currency')),
                    'base_price'       => $this->config->get('config_currency') .($product['base_price'] ? $this->currency->onlyFormat($product['base_price'],$this->config->get('config_currency')):$this->currency->format($product['price'] , $this->config->get('config_currency'))),
					'price'    		   => $order_info['currency_code'] . ($product['currency_price'] ? $this->currency->onlyFormat($product['currency_price'],$order_info['currency_code']): $this->currency->format($product['price'] , $order_info['currency_code'], $order_info['currency_value'])),
                    'base_total'       => $this->config->get('config_currency').($product['base_total'] ? $this->currency->onlyFormat($product['base_total'],$this->config->get('config_currency')):  $this->currency->format($product['total'] ,$this->config->get('config_currency'))),
					'total'    		   => $order_info['currency_code'] . ($product['currency_total'] ? $this->currency->onlyFormat($product['currency_total'],$order_info['currency_code']): $this->currency->format($product['total'] , $order_info['currency_code'], $order_info['currency_value'])),
            ...
            $paypal_data = false;
            if($order_info['payment_code'] == 'pp_standard' ){
                $paypal_data = $this->model_sale_order->getPaypalDetail($order_info['order_id']);
            }
            $this->data['paypal_data'] = $paypal_data;
            
            
            //children
            $_children_data = array();
            if($order_info['is_parent'] == 1){
                $_children = $this->model_sale_order->getOrders(array('parent_id'=>$order_info['order_id']));
                foreach($_children as $_item ){
                    $_action = array();
            ...
            $this->data['children'] = $_children_data;
            
            //主订单
            if($order_info['parent_id']){
                    $parent_order = $this->model_sale_order->getOrder($order_info['parent_id']);
                    $parent_order_action = array();
                    $parent_order_action[] = array(
                        'text' => $this->language->get('text_view'),
                        'href' => $this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $parent_order['order_id'] , 'SSL')
                    );
                    ...
                    $_children_data = array();
                        ...
                        $_children = $this->model_sale_order->getOrders(array('parent_id'=>$order_info['parent_id']));
                        foreach($_children as $_item ){
                            $_action = array();
                    ...
                    $parent_order_data = array(
                        ...
                        'store_id'      =>$parent_order['store_id'],
                        'store_from'      =>$store_array[$parent_order['store_id']],
                        'send_status'      => $send_status[$parent_order['send_forecast_status']],
                        'order_number'      => $parent_order['order_number'],
                        'customer'      => $parent_order['customer'],
                        'email'        => $parent_order['email'],
                        'status'        => $parent_order['status'],
                        'currency_code'        => $parent_order['currency_code'],
                        'base_total'         => $this->currency->format($parent_order['total'],$this->config->get('config_currency')),
                        'total'         =>  $this->currency->onlyFormat($parent_order['grand_total'],$parent_order['currency_code']),
                        'date_added'    => date($this->language->get('date_format_short'), strtotime($parent_order['date_added'])),
                        'date_modified' => date($this->language->get('date_format_short'), strtotime($parent_order['date_modified'])),
                        'selected'      => isset($this->request->post['selected']) && in_array($parent_order['order_id'], $this->request->post['selected']),
                        'action'        => $parent_order_action,
                        'payment_method' => $parent_order['payment_method'],
                        'payment_code' => $parent_order['payment_code'],
                        'children' => $_children_data,
                    );
                    $this->data['parent_order'] = $parent_order_data;
            }
	...
	public function createInvoiceNo() {
		...
		if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($invoice_no) {
                $url =$this->url->link('sale/order/ViewInvoice','token=' . $this->session->data['token'].'&order_id='.$this->request->get['order_id']);
				$json['invoice_no'] = "<a href=' ". $url ." '>".$invoice_no."</a>";
			} else {
	...
	public function history() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!$this->data['error']) { 
				$rs = $this->model_sale_order->addOrderHistory($this->request->get['order_id'], $this->request->post);
                if($rs == 1){
                    $this->data['success'] = $this->language->get('text_success');
                }else{
                    $this->data['error'] = "修改错误";
                    if($rs == -2){
                        $this->data['error'] = "库存不足";
                    }
                }
			}
		}
	...
	public function getOrderFilter(){
        ...
        if(isset($this->request->get['filter_order_number'])){
            ...
            $this->load->model('sale/order');
            $_order = $this->model_sale_order->getOrderByNo($filter_order_number);
            if($_order['parent_id']){
                $parent_id = $_order['parent_id'];
                $filter['filter_order_id'] = $parent_id;
                unset($filter['filter_order_number']);
            }else{
                $order_number = $_order['order_number'];
                $filter['filter_order_number'] = $order_number;
        ...
        if(isset($this->request->get['filter_order_id'])){
            ...
            $this->load->model('sale/order');
            $_order = $this->model_sale_order->getOrder($filter_order_id);
            if($_order['parent_id']){
                $parent_id = $_order['parent_id'];
                $filter['filter_order_id'] = $parent_id;
            }else{
                $filter['filter_order_id'] = $_order['order_id'];
            }
        }
		return $filter;
	}
     public function canShip(){
        ...
        $product_info = $this->model_catalog_product->getProductBySkuAndLang($product_sku,1);
        $_result  = array();
        $_result['flag'] = 0;
        $_result['msg'] = '';
        if($product_info){
    ...
    public function addOrderProduct(){
        $user_id =$this->session->data['user_id'];
        $store_to_lang =array('0'=>'1','52'=>'4','53'=>'6','54'=>'5','55'=>'7','56'=>'8');
        $this->load->model('sale/order');
        $this->load->model('catalog/product');
        $this->load->model('localisation/currency');
        //$order_id =$this->request->post['order_id']?$this->request->post['order_id']:0;
        $product_sku =$this->request->post['product_sku']?$this->request->post['product_sku']:'';
        $qty =$this->request->post['quantity']?$this->request->post['quantity']:1;
        $price =$this->request->post['price']?$this->request->post['price']:0;
        $store_id = $this->request->post['store_id']?$this->request->post['store_id']:0;
        $curreny_code = $this->request->post['curreny_code']?$this->request->post['curreny_code']:'';
        $json =array();
        $json['error'] =0;
        $json['message'] ='';
        if(!$product_sku){
            $json['error']=1;
            $json['message'] ="请输入你想加入的商品!";
        }
        else{
             $curreny_info =$this->model_localisation_currency->getCurrencyByCode($curreny_code);
             $product_info =$this->model_catalog_product->getProductBySkuAndLang($product_sku,$store_to_lang[$store_id]);
             $product_id = $product_info['product_id'];
        }
        //unset($this->session->data[$user_id."_new_order"]);exit;
        unset($this->session->data['normal_ship_cost']);
        unset( $this->session->data[$user_id."_new_order"]['order_total']);
        if(!$json['error']){
            $_have_product_info =$this->session->data[$user_id."_new_order"]['order_products'];
            if($_have_product_info&&$_have_product_info[$product_id]){
                $current_qty =$qty+$_have_product_info[$product_id]['quantity'];
            }else{
                 $current_qty =$qty;
            }
            $add_product_info['product_id'] =$product_id;
            $add_product_info['quantity'] =$current_qty;
            $add_product_info['name'] =$product_info['name'];
            $add_product_info['model'] =$product_info['model'];
            $add_product_info['original_price'] =$product_info['price'];
            $add_product_info['price'] =round($price,2);
            $add_product_info['total'] =round($price*$current_qty,2);
            $add_product_info['base_price'] =round($price,2);
            $add_product_info['base_total'] =round($price*$current_qty,2);
            $add_product_info['currency_price'] =round($price*$curreny_info['value'],2);
            $add_product_info['currency_total'] =round($add_product_info['currency_price']*$current_qty,2);
            $add_product_info['weight'] =$this->model_catalog_product->getProductValue('weight',$product_id);
            $add_product_info['battery_type'] =$this->model_catalog_product->getProductValue('battery_type',$product_id);
            $this->session->data[$user_id."_new_order"]['order_products'][$product_id] =$add_product_info;
            $this->data['order_products'] =$this->session->data[$user_id."_new_order"]['order_products'] ;
            $this->data['curreny_code'] = $curreny_code;
            $this->session->data[$user_id."_new_order"]['curreny_code'] =$curreny_code;
            $this->template = 'sale/include/order_product.tpl';
            $this->data['curreny_code'] = $curreny_code;
            $json['product_coutent'] = $this->render();
            $this->response->setOutput(json_encode($json));
        }
        $this->response->setOutput(json_encode($json));
    }
    ...
    public function delOrderProduct(){
        ...
        $json['error'] =0;
        $json['message'] ='';
        $add_product_id =isset($this->request->get['add_product_id'])?$this->request->get['add_product_id']:0;
        if(!$add_product_id){
            $json['error'] =1;
            $json['message'] ='请选择正确的商品!';
        }
        else{
            unset( $this->session->data[$user_id."_new_order"]['order_total']);
            unset($this->session->data[$user_id."_new_order"]['order_products'][$add_product_id]);
            $this->data['order_products'] =$this->session->data[$user_id."_new_order"]['order_products'];
            $this->data['curreny_code'] = $this->session->data[$user_id."_new_order"]['curreny_code'];
            $this->template = 'sale/include/order_product.tpl';
    ...
    public function AddOrderTotals(){
        ...
        $order_products = $this->session->data[$user_id."_new_order"]['order_products'];
        $subtotal =0;
        $currency_subtotal =0;
        foreach ($order_products as $order_product) {
            $currency_subtotal +=$order_product['currency_total'];
            $subtotal +=$order_product['total'];
        }
         
        $symbolleft =$this->currency->getSymbolLeft($this->session->data[$user_id."_new_order"]['curreny_code']);       
        $text =$symbolleft.$currency_subtotal;
        $total_array=array(
                            'text'=>$text,
                            'value'=>$subtotal,
                            'title' =>'Subtotal',
        ...
        $this->session->data[$user_id."_new_order"]['order_total']['sub_total'] =$total_array;
        $total_data = $this->session->data[$user_id."_new_order"]['order_total'];
        $this->data['order_totals']=$total_data;
        $this->template = 'sale/include/order_total.tpl';
        $json['total_coutent'] = $this->render(); 

        $this->response->setOutput(json_encode($json));

    }
     public function addShippingTotal(){
         
        $user_id =$this->session->data['user_id'];
        //unset($this->session->data[$user_id."_new_order"]['order_total']);exit;
        $json =array();
        $json['error'] =0;
        $json['message'] ='';
        $shipping_cost =$this->request->get['cost']?$this->request->get['cost']:0;
        $shipping_method =$this->request->get['method']?$this->request->get['method']:'Shipping Cost';
        $order_id =isset($this->request->get['order_id'])?$this->request->get['order_id']:($user_id."9999999");
        $pk =$this->request->get['pk']?$this->request->get['pk']:'';
        if(!$shipping_cost||!$pk){
            $json['error'] =1;
            $json['message'] ='请选择正确的运送方式!';
        }
        else{
            //添加运费数据
           $this->load->model('sale/order');
           ...
           $this->session->data['normal_ship_cost'][$pk] =$shipping_cost;
           $total_normal_ship_cost =0;
           foreach($this->session->data['normal_ship_cost'] as $shipping_price){
                $total_normal_ship_cost +=$shipping_price;
           }
           $text =$this->currency->format($total_normal_ship_cost,$this->session->data[$user_id."_new_order"]['curreny_code']);
           $total_data =array(
                 'code'=>'shipping',
                 'title'  =>$shipping_method,
                 'text'=>$text,
                 'value'=>$total_normal_ship_cost, 
                  'sort_order' =>'3'
             ) ; 
            $this->session->data[$user_id."_new_order"]['order_total']['shipping'] =$total_data;
            $order_totals = $this->session->data[$user_id."_new_order"]['order_total'];
            ...
            $sub_total_data['code'] ='total';
            $sub_total_data['title'] ='Grand Total';
            $sub_total_data['text'] =$this->currency->format($sub,$this->session->data[$user_id."_new_order"]['curreny_code']);
            $sub_total_data['value'] =$sub;
            $sub_total_data['sort_order'] =9;
            $this->session->data[$user_id."_new_order"]['order_total']['total'] =$sub_total_data;
    ...
    //发送未付款的提醒邮件
     public function send_pending_email(){
        ...
        $language->load('mail/pending_email');
        $email_data =array();
        ...
        $template->data['text_main_content'] = sprintf($language->get('text_main_content'),$order_info['order_number']);
        $email_data['email_subject'] =sprintf($language->get('text_title'),$order_info['order_number']);
        $html = $template->fetch('mail/pending_email.tpl');
        $email_data['email_content'] =addslashes($html);
        $email_data['is_html'] =1;
        $email_data['attachments'] ="";
        $email_id =$this->model_tool_email->addEmailList($email_data);
        if($email_id){
            $this->session->data['success'] = "付款提醒邮件发送成功";
            $this->db->query("UPDATE ".DB_PREFIX."order set pending_email_send=1,pending_email_send_time=NOW() where order_id=".$order_id);
			$this->redirect($this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $order_id, 'SSL'));
        }
        else{
             $this->session->data['success'] = "付款提醒邮件发送失败";
			$this->redirect($this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $order_id, 'SSL'));
        }
     }
    //大额信用卡验证
    public function credit_validate_email(){
        $this->load->model('tool/email');
        $this->load->model('sale/order');
        $order_id=$this->request->get['order_id'] ;
        $order_info = $this->getOrderInfo($order_id);
        $language =$this->model_sale_order->load_language($order_info['store_id']);
        $language->load('mail/credit_validate_email');
        $email_data =array();
        $email_data['store_id'] =$order_info['store_id'];
        $email_data['email_from'] ='MyLED';
        $email_data['email_to'] =$order_info['email'];
        ...
        $template->data['title'] =sprintf($language->get('text_title'),$order_info['order_number']); 
        $template->data['text_main_content'] = sprintf($language->get('text_main_content'),$order_info['order_number']);
        $template->data['text_main_content_address'] = $language->get('text_main_content_address');
        $template->data['text_main_content_product'] = $language->get('text_main_content_product');
        $template->data['text_main_content_2'] = $language->get('text_main_content_2');
        ...
        $template->data['text_order_total'] = $language->get('text_order_total');
        $template->data['text_order_shipping_cost'] = $language->get('text_order_shipping_cost');
        $template->data['text_order_grand_total'] = $language->get('text_order_grand_total');
        $email_data['email_subject'] =sprintf($language->get('text_title'),$order_info['order_number']);
        ...
        $html = $template->fetch('mail/credit_validate_email.tpl');
        $email_data['email_content'] =addslashes($html);
        $email_data['is_html'] =1;
        $email_data['attachments'] ="";
        $email_id =$this->model_tool_email->addEmailList($email_data);
        if($email_id){
            $this->session->data['success'] = "大额信用卡邮件发送成功";
            $this->db->query("UPDATE ".DB_PREFIX."order set credit_validate_email_send=1,credit_validate_email_send_time=NOW() where order_id=".$order_id);
			$this->redirect($this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $order_id, 'SSL'));
        }
        else{
             $this->session->data['success'] = "大额信用卡邮件发送失败";
			$this->redirect($this->url->link('sale/order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $order_id, 'SSL'));
        }
     }
