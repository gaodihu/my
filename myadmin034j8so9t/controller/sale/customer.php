...
class ControllerSaleCustomer extends Controller { 
	...
	protected function getForm() {
        ...
        $this->data['reset_password'] =$this->url->link('sale/customer/reset_password', 'token=' . $this->session->data['token'] . '&customer_id=' . $this->request->get['customer_id'], 'SSL');
		...
		if (isset($this->request->post['address'])) { 
			...
			$this->data['addresses'] = $this->model_sale_customer->getAddresses($this->request->get['customer_id'],'shipping');
		} else {
			$this->data['addresses'] = array();
		}

        if (isset($this->request->post['b_address'])) {
            $this->data['b_address'] = $this->request->post['b_address'];
        } elseif (isset($this->request->get['customer_id'])) {
            $bliing_address =$this->model_sale_customer->getAddresses($this->request->get['customer_id'],'billing');
            $this->data['b_address'] =array_shift($bliing_address);

        } else {
            $this->data['b_address'] = array();
        }
    ...
    public function shipping_method() {
		...
		$json = array();
        $address_info =array();
        $address_info['firstname'] =isset($this->request->post['firstname'])?$this->request->post['firstname']:'';
        $address_info['lastname'] =isset($this->request->post['lastname'])?$this->request->post['lastname']:'';
        $address_info['company'] =isset($this->request->post['company'])?$this->request->post['company']:'';
        $address_info['tax_id'] =isset($this->request->post['tax_id'])?$this->request->post['tax_id']:'';
        $address_info['address_1'] =isset($this->request->post['address_1'])?$this->request->post['address_1']:'';
        $address_info['address_2'] =isset($this->request->post['address_2'])?$this->request->post['address_2']:'';
        $address_info['postcode'] =isset($this->request->post['postcode'])?$this->request->post['postcode']:'';
        $address_info['city'] =isset($this->request->post['city'])?$this->request->post['city']:'';
        $address_info['zone_id'] =isset($this->request->post['zone_id'])?(int)$this->request->post['zone_id']:'';
        $address_info['phone'] =isset($this->request->post['phone'])?(int)$this->request->post['phone']:'';
        $address_info['country_id'] =isset($this->request->post['country_id'])?(int)$this->request->post['country_id']:'';
        ...
        $ship_data =$this->model_shipping_myled->getShippingMethod($address_info,$total_weigth,$products);
        $is_remote =$this->model_shipping_myled->isRemoteArea($address_info);
        $this->data['is_remote'] =$is_remote;
        $this->data['can_shipping'] =$ship_data['can_shipping'];
        
        $this->data['no_shipping'] =$ship_data['no_shipping'];
        $this->data['curreny_code'] =$this->session->data[$user_id."_new_order"]['curreny_code'];
        $this->load->model('localisation/order_status');
		$this->data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses();
        $this->template = 'sale/include/order_product_total.tpl';

		unset($this->session->data[$user_id."_new_order"]['order_total']);

        $json['content'] = $this->render();
        $this->response->setOutput(json_encode($json));
        
	}

    public function reset_password(){
        $customer_id =$this->request->get['customer_id']?(int)$this->request->get['customer_id']:'';
        if(!$customer_id){
            $this->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'], 'SSL'));
        }
        $this->load->helper('fun.inc');
        $password =getRandomStr(10);
        $salt =getRandomStr(2);
        $res =$this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt) . "', password = '" . $this->db->escape(md5($salt.$password).":".$salt) . "' WHERE customer_id = '" .$customer_id . "'");
        if($res){
            ...
            $email_data =array();
            $customer_info =$this->model_sale_customer->getCustomer($customer_id);
            $language =$this->model_sale_order->load_language($customer_info['store_id']);
            if($customer_info['store_id']==0){
                $store_info['name'] ="www.myled.com";
                $store_info['url'] ="https://www.myled.com";
            }else{
                $store_info =$this->model_setting_store->getStore($customer_info['store_id']);
            }
            $language->load('mail/reset_password');
            $template = new Template();
            $template->data['store_id'] = $customer_info['store_id'];
            $template->data['store_name'] = $store_info['name'];
            $template->data['store_url'] = $store_info['url']."/";
            ...
            $template->data['text_main_content'] = sprintf($language->get('text_main_content'),$password,$store_info['url']."/index.php?route=account/account");
            $email_data['email_subject'] =$language->get('text_title');
            $email_data['store_id'] =$customer_info['store_id'];
            $email_data['email_from'] ='MyLED ';
            $email_data['email_to'] =$customer_info['email'];

            $html = $template->fetch('mail/reset_password.tpl');
            $email_data['email_content'] =addslashes($html);
            $email_data['is_html'] =1;
            $email_data['attachments'] ='';
            $this->model_tool_email->addEmailList($email_data);
            $this->session->data['success']="reset password successful!";
            $this->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'], 'SSL'));
        }
        else{
            $this->error['warning'] ="reset failed! please try again !";
            $this->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'], 'SSL'));
        }
