...
class ControllerSaleCoupon extends Controller {
	...
	protected function getList() {
        ...
        if (isset($this->request->get['filter_code'])) {
			$filter['filter_code'] = $this->request->get['filter_code'];
		} else {
			$filter['filter_code'] = '';
		}
        if (isset($this->request->get['filter_start_from'])) {
			$filter['filter_start_from'] = $this->request->get['filter_start_from'];
		} else {
			$filter['filter_start_from'] = '';
		}
        if (isset($this->request->get['filter_start_to'])) {
			$filter['filter_start_to'] = $this->request->get['filter_start_to'];
		} else {
			$filter['filter_start_to'] = '';
		}
        if (isset($this->request->get['filter_end_from'])) {
			$filter['filter_end_from'] = $this->request->get['filter_end_from'];
		} else {
			$filter['filter_end_from'] = '';
		}
        if (isset($this->request->get['filter_end_to'])) {
			$filter['filter_end_to'] = $this->request->get['filter_end_to'];
		} else {
			$filter['filter_end_to'] = '';
		}
		if (isset($this->request->get['sort'])) {
		...
		foreach ($results as $result) {
            ...
            $action[] = array(
				'text' => 'copy',
				'href' => $this->url->link('sale/coupon/copy', 'token=' . $this->session->data['token'] . '&coupon_id=' . $result['coupon_id']."&act=copy" . $url, 'SSL')
			);
			...
			$this->data['coupons'][] = array(
				...
				'date_start' => date('Y-m-d H:i:s', strtotime($result['date_start'])),
				'date_end'   => date('Y-m-d H:i:s', strtotime($result['date_end'])),
        ...
        $this->data['sort_id'] = HTTPS_SERVER . 'index.php?route=sale/coupon&token=' . $this->session->data['token'] . '&sort=coupon_id' . $url;
	...
	protected function getForm() {
        ...
        if (isset($this->error['front_name'])) {
			$this->data['error_front_name'] = $this->error['front_name'];
		} else {
			$this->data['error_front_name'] = '';
		}
        ...
        if (isset($this->error['error_sku_in_catalog'])) {
			$this->data['error_sku_in_catalog'] = $this->error['error_sku_in_catalog'];
		} else {
			$this->data['error_sku_in_catalog'] = '';
		}	
		...
		} elseif(!isset($this->request->get['act'])) {
        ...
        elseif(isset($this->request->get['act'])&&$this->request->get['act']=='copy'){
            $this->data['action'] = $this->url->link('sale/coupon/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
        }
        ...
        if (isset($this->request->post['front_name'])) {
			$this->data['front_name'] = $this->request->post['front_name'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$this->data['front_name'] = $this->model_sale_coupon->getCouponDescription($this->request->get['coupon_id']);;
		} else {
			$this->data['front_name'] = '';
		}
        ...
        if (isset($this->request->post['buy_x'])) {
			$this->data['buy_x'] = $this->request->post['buy_x'];
		} elseif (!empty($coupon_info)) {
			$this->data['buy_x'] = $coupon_info['buy_x'];
		} else {
			$this->data['buy_x'] = '';
		}
        ...
        if (isset($this->request->post['condition_total'])) {
			$this->data['condition_total'] = $this->request->post['condition_total'];
		} elseif (!empty($coupon_info)) {
			$this->data['condition_total'] = $coupon_info['condition_total'];
		} else {
			$this->data['condition_total'] = 0;
		}

        if (isset($this->request->post['total_qty'])) {
			$this->data['total_qty'] = $this->request->post['total_qty'];
		} elseif (!empty($coupon_info)) {
			$this->data['total_qty'] = $coupon_info['total_qty'];
		} else {
			$this->data['total_qty'] =0;
		}
         if (isset($this->request->post['condition_total_qty'])) {
			$this->data['condition_total_qty'] = $this->request->post['condition_total_qty'];
		} elseif (!empty($coupon_info)) {
			$this->data['condition_total_qty'] = $coupon_info['condition_total_qty'];
		} else {
			$this->data['condition_total_qty'] =0;
		}

        if (isset($this->request->post['row_item_qty'])) {
			$this->data['row_item_qty'] = $this->request->post['row_item_qty'];
		} elseif (!empty($coupon_info)) {
			$this->data['row_item_qty'] = $coupon_info['row_item_qty'];
		} else {
			$this->data['row_item_qty'] = 0;
		}

        if (isset($this->request->post['combine_condition'])) {
			$this->data['combine_condition'] = $this->request->post['combine_condition'];
		} elseif (!empty($coupon_info)) {
			$this->data['combine_condition'] = $coupon_info['combine_condition'];
		} else {
			$this->data['combine_condition'] = 1;
		}

        if (isset($this->request->post['combine_condition_value'])) {
			$this->data['combine_condition_value'] = $this->request->post['combine_condition_value'];
		} elseif (!empty($coupon_info)) {
			$this->data['combine_condition_value'] = explode(',',$coupon_info['combine_condition_value']);
		} else {
			$this->data['combine_condition_value'] =array();
		}
        if (isset($this->request->post['sku_condition'])) {
			$this->data['sku_condition'] = $this->request->post['sku_condition'];
		} elseif (!empty($coupon_info)) {
			$this->data['sku_condition'] = $coupon_info['sku_condition'];
		} else {
			$this->data['sku_condition'] = '';
		}

        if (isset($this->request->post['category_condition'])) {
			$this->data['category_condition'] = $this->request->post['category_condition'];
		} elseif (!empty($coupon_info)) {
			$this->data['category_condition'] = $coupon_info['category_condition'];
		} else {
			$this->data['category_condition'] = '';
		}

		if (isset($this->request->post['coupon_product'])) {
			$this->data['coupon_products'] = $this->request->post['coupon_product'];
		} elseif (isset($this->request->get['coupon_id'])) {	
            $this->load->model('catalog/product');
			$products = $this->model_sale_coupon->getCouponProducts($this->request->get['coupon_id']);
            $coupon_products=array();
            foreach($products as $product_id){
                $coupon_products[] =$this->model_catalog_product->getProductValue('model',$product_id);
            }
            $this->data['coupon_products'] =implode(',',$coupon_products);
		} else {
		...
		if (isset($this->request->post['date_start'])) {
			...
			$this->data['date_start'] = date('Y-m-d H:i:s', strtotime($coupon_info['date_start']));
		} else {
			$this->data['date_start'] = date('Y-m-d H:i:s', time());
		}

		if (isset($this->request->post['date_end'])) {
			$this->data['date_end'] = $this->request->post['date_end'];
		} elseif (!empty($coupon_info)) {
			$this->data['date_end'] = date('Y-m-d H:i:s', strtotime($coupon_info['date_end']));
		} else {
			$this->data['date_end'] = date('Y-m-d H:i:s', time());
		}
	...
	protected function validateForm() {
        ...
        if ((utf8_strlen($this->request->post['coupon_description'][1]['front_name']) < 3) || (utf8_strlen($this->request->post['coupon_description'][1]['front_name']) > 255)) {
			$this->error['front_name'] = $this->language->get('error_front_name');
		}
        ...
        if(isset($this->request->post['coupon_category'])&&isset($this->request->post['coupon_product'])) {
            if($this->request->post['sku_condition']==1){
                $error_product =array();
                $this->load->model('catalog/product');
                foreach(explode(',',$this->request->post['coupon_product']) as $sku){
                    $error =1;
                    $catagorys =$this->model_catalog_product->getProductCategoriesBySku($sku);
                    foreach($catagorys as $catalog_id){
                        foreach($this->request->post['coupon_category'] as $in_catalog_id){
                            if($catalog_id==$in_catalog_id){
                                $error=0;
                            }
                        }
                    }
                    if($error==1){
                        $error_product[] =$sku;
                        $this->error['error_sku_in_catalog'] = $sku."不在任何一个分类中";
                    }
                }
            }
        ...
        if(isset($error_product)){
            $this->error['error_sku_in_catalog'] = implode(',',$error_product)."不在任何一个分类中";
        }
		if (!$this->error) {
			return true;
		} else {
