...
class ControllerPaymentPPExpress extends Controller {
	...
	public function index() {
        ...
        //小额
        if (isset($this->request->post['pp_express_small_enabled'])) {
			$this->data['pp_express_small_enabled'] = $this->request->post['pp_express_small_enabled'];
		} else {
			$this->data['pp_express_small_enabled'] = $this->config->get('pp_express_small_enabled');
		}
        if (isset($this->request->post['pp_express_small_limit'])) {
			$this->data['pp_express_small_limit'] = $this->request->post['pp_express_small_limit'];
		} else {
			$this->data['pp_express_small_limit'] = $this->config->get('pp_express_small_limit');
		}
        
        if (isset($this->request->post['pp_express_username_small'])) {
			$this->data['pp_express_username_small'] = $this->request->post['pp_express_username_small'];
		} else {
			$this->data['pp_express_username_small'] = $this->config->get('pp_express_username_small');
		}

		if (isset($this->request->post['pp_express_password_small'])) {
			$this->data['pp_express_password_small'] = $this->request->post['pp_express_password_small'];
		} else {
			$this->data['pp_express_password_small'] = $this->config->get('pp_express_password_small');
		}

		if (isset($this->request->post['pp_express_signature_small'])) {
			$this->data['pp_express_signature_small'] = $this->request->post['pp_express_signature_small'];
		} else {
			$this->data['pp_express_signature_small'] = $this->config->get('pp_express_signature_small');
		}
	...
	protected function validate() {
        ...
        //小额
         ...
         if ($this->request->post['pp_express_small_enabled'] != ''  && intval($this->request->post['pp_express_small_enabled']) == 1) {
			if (empty($this->request->post['pp_express_small_limit']) || floatval($this->request->post['pp_express_small_limit'])<=0) {
                $this->error['small_limit'] = "小额账号最高限制(美元)错误";
            }
            
            if (empty($this->request->post['pp_express_username_small'])) {
                $this->error['username_small'] = $this->language->get('error_username');
            }
        
            if (empty($this->request->post['pp_express_password_small'])) {
                $this->error['password_small'] = $this->language->get('error_password');
            }

            if (empty($this->request->post['pp_express_signature_small'])) {
                $this->error['signature_small'] = $this->language->get('error_signature');
            }
		}
