...
class ControllerCatalogAttribute extends Controller { 
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $attribute_id) {
				$rs = $this->model_catalog_attribute->deleteAttribute($attribute_id);
    ...
    public  function delOption(){
        ...
        if (isset($this->request->get['option_id'])) {
            ...
            $this->model_catalog_attribute->deleteOptionValue($option_id);
            
        }
        $this->redirect($this->url->link('catalog/attribute/update', 'token=' . $this->session->data['token']."&attribute_id=".$attr_id, 'SSL'));
    }
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$this->data['attributes'][] = array(
				...
				'name'            => $result['name'],
				'attribute_group_code' => implode(',',$result['attribute_group_code']),
	...
	protected function getForm() {
		...
		//属性值集合
		if (isset($this->request->post['attribute_option'])) {
			$this->data['attribute_option'] = $this->request->post['attribute_option'];
		} elseif (isset($this->request->get['attribute_id'])) {
			$this->data['attribute_option'] = $this->model_catalog_attribute->getAttributeOptions($this->request->get['attribute_id']);
		} else {
			$this->data['attribute_option'] = array();
		}
		...
		if (isset($this->request->post['attribute_group_id'])) {
			...
			$attribute_group_arr = $this->model_catalog_attribute_group->getAttributeToGroup($attribute_info['attribute_id']);
			foreach($attribute_group_arr as $attribute){
				$this->data['attribute_group_id'][] =$attribute['attribute_group_id'];
			}
		} else {
			$this->data['attribute_group_id'] = array();
		}
		...
		if (isset($this->request->post['value_type'])) {
			$this->data['value_type'] = $this->request->post['value_type'];
		} elseif (!empty($attribute_info)) {
			$this->data['value_type'] = $attribute_info['value_type'];
		} else {
		...
		if (isset($this->request->post['unit'])) {
			...
			$this->data['unit'] = $attribute_info['unit'];
		} else {
			$this->data['unit'] = "";
		}
		...
		if (isset($this->request->post['attribute_code'])) {
			$this->data['attribute_code'] = $this->request->post['attribute_code'];
		} elseif (!empty($attribute_info)) {
			$this->data['attribute_code'] = $attribute_info['attribute_code'];
		} else {
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['attribute_description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 0) || (utf8_strlen($value['name']) > 64)) {
				$this->error['name'][$language_id] = $this->language->get('error_name');
			}
		}

		if(empty($this->request->post['attribute_code'])){
			$this->error['warning'] = $this->language->get('error_code');
		}
		if(empty($this->request->post['value_type']) || !in_array($this->request->post['value_type'],array('numerical','text','radio','option'))){
			$this->error['warning'] = "请选择‘属性值类型’";
		}
