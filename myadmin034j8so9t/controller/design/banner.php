...
class ControllerDesignBanner extends Controller {
	...
	public function update() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->redirect($this->url->link('design/banner/update', 'token=' . $this->session->data['token']."&banner_id=". $this->request->get['banner_id']. $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			$this->data['banners'][] = array(
				...
				'name'      => $result['name'],
				'banner_code'      => $result['banner_code'],
				'banner_width'      => $result['banner_width'],
				'banner_height'      => $result['banner_height'],
		...
		$this->data['column_banner_code'] = $this->language->get('banner_code');
	...
	protected function getForm() {
		...
		if (isset($this->error['banner_code'])) {
			$this->data['error_banner_code'] = $this->error['banner_code'];
		} else {
			$this->data['error_banner_code'] = '';
		}
		if (isset($this->error['banner_width'])) {
			$this->data['error_banner_width'] = $this->error['banner_width'];
		} else {
			$this->data['error_banner_width'] = '';
		}
		if (isset($this->error['banner_height'])) {
			$this->data['error_banner_height'] = $this->error['banner_height'];
		} else {
			$this->data['error_banner_height'] = '';
		}
        ...
        //分类页头部banner需要分类目存放
        if(isset($banner_info)&&$banner_info['banner_code']=='catagory_top_banner'){
            $this->load->model('catalog/category');
            $all_catagory = $this->model_catalog_category->getCategories();
            $this->data['all_catagory'] = $all_catagory ;
        }
		...
		if (isset($this->request->post['banner_code'])) {
			$this->data['banner_code'] = $this->request->post['banner_code'];
		} elseif (!empty($banner_info)) {
			$this->data['banner_code'] = $banner_info['banner_code'];
		} else {
			$this->data['banner_code'] = '';
		}
		if (isset($this->request->post['banner_width'])) {
			$this->data['banner_width'] = $this->request->post['banner_width'];
		} elseif (!empty($banner_info)) {
			$this->data['banner_width'] = $banner_info['banner_width'];
		} else {
		...
		if (isset($this->request->post['banner_height'])) {
			...
			$this->data['banner_height'] = $banner_info['banner_height'];
		} else {
		...
		foreach ($banner_images as $key =>$banner_image_info) {
			foreach($banner_image_info as $banner_image){
				if ($banner_image['image'] && file_exists(DIR_IMAGE . $banner_image['image'])) {
					$image = $banner_image['image'];
				} else {
					$image = 'no_image.jpg';
				}			
				$this->data['banner_images'][$key][] = array(
                    ...
                    'category_id'                    => $banner_image['category_id'],
                    'start_time'               => $banner_image['start_time'],
                    'end_time'                 => $banner_image['end_time'],
