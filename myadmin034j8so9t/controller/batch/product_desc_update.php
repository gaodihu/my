<?php 
class ControllerBatchProductDescUpdate extends Controller {
	private $error = array();
	public function index() {
		
		$this->document->setTitle($this->language->get('商品描述板块批量翻译')); 
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => "商品描述板块批量翻译",
			'href'      => $this->url->link('batch/product_desc_update', 'token=' . $this->session->data['token'], 'SSL'),       		
			'separator' => ' :: '
		);

		$this->data['download'] = $this->url->link('batch/product_desc_update/download', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['upload'] = $this->url->link('batch/product_desc_update/upload', 'token=' . $this->session->data['token'], 'SSL');	
	    ...
	    $this->template = 'batch/product_desc_update.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
    ...
    public function upload(){
		...
		if(!in_array($file_type,$accpet_file )){
            ...
            $this->redirect($this->url->link('batch/product_desc_update', 'token=' . $this->session->data['token'], 'SSL'));
		}
		else{
                    ...
                    if(!$query_product_exist->num_rows){
                        ...
                        if(!$query_product_exist->num_rows){
                            ...
                            echo "<p style='color:red'>第".$i."行SKU不存在描述和标题信息，请完善</p>";
                             ...
                             if($info[8]){
                                $video ='<iframe frameborder="0" height="350" src="'.trim($info[8]).'" width="425"></iframe>';
                            }else{
                                $video='';
                            }
                            for($k=2;$k<=9;$k++){
                            ...
                            $update_str =array();
                            if($info[2]){
                                $update_str[]="packaging_list='".$this->db->escape(trim($info[2]))."'";
                            }
                            if($info[3]){
                                $update_str[]="read_more='".$this->db->escape(trim($info[3]))."'";
                            }
                            if($info[4]){
                                $update_str[]="application_image='".$this->db->escape(trim($info[4]))."'";
                            }
                            if($info[5]){
                                $update_str[]="size_image='".$this->db->escape(trim($info[5]))."'";
                            }
                            if($info[6]){
                                $update_str[]="features='".$this->db->escape(trim($info[6]))."'";
                            }
                            if($info[7]){
                                $update_str[]="installation_method='".$this->db->escape(trim($info[7]))."'";
                            }
                            if($info[8]){
                                $update_str[]="video='".$this->db->escape($video)."'";
                            }
                            if($info[9]){
                                $update_str[]="notes='".$this->db->escape(trim($info[9]))."'";
                            }
                            $sql_update .= implode(',',$update_str);
                            $sql_update .=" where product_id= ".$product_id." and language_id=".(int)$info[1];
    ...
    public function download(){
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('C1', 'Packaging List');
        $objPHPExcel->getActiveSheet()->setCellValue('D1', 'Read More');
        $objPHPExcel->getActiveSheet()->setCellValue('E1', 'Application Image');
        $objPHPExcel->getActiveSheet()->setCellValue('F1', 'Size Image');
        $objPHPExcel->getActiveSheet()->setCellValue('G1', 'Features');
        $objPHPExcel->getActiveSheet()->setCellValue('H1', 'Installation Method/Usage');
        $objPHPExcel->getActiveSheet()->setCellValue('I1', 'Video');
        $objPHPExcel->getActiveSheet()->setCellValue('J1', 'Notes');
		$objPHPExcel->getActiveSheet()->setTitle("商品描述板块批量翻译模板");
        $objPHPExcel->setActiveSheetIndex(0);
         //Redirect output to a client’s web browser (Excel2007)
        header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        header("Content-Disposition: attachment;filename=商品描述板块批量翻译模板.xlsx" );
