<?php 
class ControllerDataDelBanner extends Controller {
	private $error = array(); 
	public function index() {
		
		$this->document->setTitle("批量删除banner信息");
		...
		$this->data['breadcrumbs'][] = array(
			'text'      => "批量删除banner信息",
			'href'      => $this->url->link('data/del_banner', 'token=' . $this->session->data['token'], 'SSL'),       		
			'separator' => ' :: '
		);
		$this->data['upload'] = $this->url->link('data/del_banner/upload', 'token=' . $this->session->data['token'], 'SSL');
	    ...
	    $this->template = 'data/del_banner.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
    ...
    public function upload(){
		...
		if(!in_array($file_type,$accpet_file )){
            ...
            $this->redirect($this->url->link('data/del_banner', 'token=' . $this->session->data['token'], 'SSL'));
		}
		else{
			...
			if($_FILES[$upload_inptu_file_name]['tmp_name']){
                    ...
                    foreach($file_content as $info){
                        ...
                        $banner_img_id =trim($info[0]);
                        if($banner_img_id){
                            $sql ="delete from ".DB_PREFIX."banner_image_description where banner_image_id=".$banner_img_id;
                            $res =$this->db->query($sql);
                            if(!$res){
                                $this->error[$i] =$i;
                                echo "<p style='color:red'>第".$i."行banner删除失败，请检查！</p>";
                            }
                        }
                       $i++;
                     }
                     ...
                     if($error_count>0){
                        $back_url =  $this->url->link('data/del_banner', 'token=' . $this->session->data['token'], 'SSL');  
