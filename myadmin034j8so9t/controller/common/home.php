...
class ControllerCommonHome extends Controller {   
	public function index() {
		...
		$this->data['total_order'] = $this->model_sale_order->getTotalOrders(array('parent_id' => 0));
	...
	public function chart() {
		...
		switch ($range) {
			case 'day':
                $_now_start_time = strtotime(date('Y-m-d H').':00:00') + 3600;
				for ($i = 0; $i < 24; $i++) {
                    $start_time = $_now_start_time - (24 - $i) * 3600;
                    $end_time   = $start_time + 3600;
                    $start_time_s = date('Y-m-d H:i:s',$start_time);
                    $end_time_s   = date('Y-m-d H:i:s',$end_time);
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id in (5,2)  AND  date_added >= '{$start_time_s}' AND  date_added < '{$end_time_s}'  AND parent_id = 0");
                    //echo "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id in (5,2)  AND  date_added >= '{$start_time_s}' AND  date_added < '{$end_time_s}'  AND parent_id = 0"."\n";
					if ($query->num_rows) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE date_added >= '{$start_time_s}' AND  date_added < '{$end_time_s}' ");
			...
			case 'week':
				...
				for ($i = 0; $i < 7; $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id in (5,2)  AND DATE(date_added) = '" . $this->db->escape($date) . "' AND parent_id = 0 GROUP BY DATE(date_added)");
			...
			case 'month':
				for ($i = 1; $i <= date('t'); $i++) {
					...
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id in (5,2) AND (DATE(date_added) = '" . $this->db->escape($date) . "') AND parent_id = 0 GROUP BY DAY(date_added)");
			...
			case 'year':
				for ($i = 1; $i <= 12; $i++) {
					$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id in (5,2)  AND YEAR(date_added) = '" . date('Y') . "' AND MONTH(date_added) = '" . $i . "' AND parent_id = 0 GROUP BY MONTH(date_added)");
	...
	public function permission() {
		if (isset($this->request->get['route'])) {
            ...
            if($this->request->get['route']==='sale/order'&&!$this->user->hasPermission('access', $route)){
